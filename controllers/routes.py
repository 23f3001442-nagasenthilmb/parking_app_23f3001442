...
from app import db
from models.models import Admin, User, ParkingLot, ParkingSpot, ReserveParkingSpot
...
def admin_summary():
    ...
    revenue_per_lot = []
    for lot in lots:
        lot_revenue = db.session.query(db.func.sum(ReserveParkingSpot.parking_cost))\
            .join(ParkingSpot, ReserveParkingSpot.spot_id == ParkingSpot.spot_id)\
            .filter(ParkingSpot.lot_id == lot.lot_id).scalar() or 0
        lot.available_spots = ParkingSpot.query.filter_by(lot_id=lot.lot_id, status='A').count()
        lot.occupied_spots = ParkingSpot.query.filter_by(lot_id=lot.lot_id, status='O').count()
        revenue_per_lot.append({'lot_id': lot.lot_id, 'location': lot.location, 'revenue': lot_revenue})

    lot_names = [lot.location for lot in lots]
    available_data = [lot.available_spots for lot in lots]
    occupied_data = [lot.occupied_spots for lot in lots]
...
def user_summary():
    ...
    occupied_spots = ParkingSpot.query.filter_by(status='O').count()
    total_spots = ParkingSpot.query.count()
    return render_template('user_summary.html', user=user, user_stats=user_stats, user_bookings=user_bookings, now=datetime.now(), available_spots=available_spots, occupied_spots=occupied_spots, total_spots=total_spots)
